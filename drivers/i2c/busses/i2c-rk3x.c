...
#define REG_MRXADDR    0x08 /* slave address for master receive */
#define REG_MRXRADDR   0x0c /* slave register address for master receive */
...
enum {
	REG_CON_MOD_TX = 0,      /* simply transmit bytes */
	REG_CON_MOD_REGISTER_TX, /* select register and transmit data */
	REG_CON_MOD_RX,          /* receive bytes */
	REG_CON_MOD_REGISTER_RX, /* select register and receive data */
};
...
#define REG_CON_ACTACK    (1 << 6) /* ??? probably if ACK was received */
...
#define REG_INT_BTF       (1 << 0)
#define REG_INT_BRF       (1 << 1)
 ...
 */
static void rk3x_i2c_set_rx_mode(struct rk3x_i2c *i2c)
{
	u32 con = i2c_readl(i2c, REG_CON);

	con &= ~(REG_CON_MOD_MASK);
	con |= REG_CON_MOD(REG_CON_MOD_RX) | REG_CON_LASTACK;
...
static void rk3x_i2c_prepare_read(struct rk3x_i2c *i2c)
{
	...
	// TODO: complain if len > 32
	// switch to plain RX mode after we got something
	if(i2c->processed != 0)
		rk3x_i2c_set_rx_mode(i2c);
...
/**
 * Setup a write according to i2c->msg
 */
static void rk3x_i2c_prepare_write(struct rk3x_i2c *i2c)
{
	unsigned int i, j;
	u32 cnt = 0;
	u32 val;
...
static void rk3x_i2c_handle_start(struct rk3x_i2c *i2c, unsigned int ipd)
{
	...
	if (i2c->mode == REG_CON_MOD_TX) {
		...
		rk3x_i2c_prepare_write(i2c);
	} else {
		/* in any other case, we are going to be reading after we finish. */
...
static void rk3x_i2c_handle_read(struct rk3x_i2c *i2c, unsigned int ipd)
{
	...
	if (!(ipd & REG_INT_MBRF)) {
		dev_err(i2c->dev, "unexpected irq in READ: 0x%x\n", ipd);
		rk3x_i2c_stop(i2c, -ENXIO);
		rk3x_i2c_clean_ipd(i2c);
		return;
	}
...
static void rk3x_i2c_set_scl_rate(struct rk3x_i2c* i2c, unsigned long scl_rate)
{
	...
	/* why is the divisor written twice? do we want to know? */
	divh = divl = DIV_ROUND_UP(div, 2);
...
static int rk3x_i2c_setup(struct rk3x_i2c *i2c, struct i2c_msg *msgs, int num)
{
	...
	/*
	 * The I2C adapter is capable of performing the transmit/receive operation
	 * required for I2C register reads in one go. The MRXADDR/MRXRADDR registers
	 * are used to hold the I2C slave address and I2C slave register address
	 * for the first master transmit operation.
	 */
	...
	if (num >= 2 && msgs[0].len < 4
		...
		reg_valid_bits = REG_MRXADDR_LOW;

		if (msgs[0].len >= 2) {
			reg_addr |= msgs[0].buf[1] << 8;
			reg_valid_bits |= REG_MRXADDR_MID;

			if(msgs[0].len >= 3) {
				reg_addr |= msgs[0].buf[2] << 16;
				reg_valid_bits |= REG_MRXADDR_HIGH;
			}
		}
		...
		i2c_writel(i2c, reg_addr | reg_valid_bits, REG_MRXRADDR);
                         ...
                         struct i2c_msg *msgs, int num)
{
	...
	rk3x_i2c_set_scl_rate(i2c, 100 * 1000);
...
static int rk3x_i2c_probe(struct platform_device *pdev)
{
	...
	if(of_property_read_u32(pdev->dev.of_node, "bus-idx", &bus_idx) != 0) {
		dev_err(&pdev->dev, "rk3x-i2c requires a 'bus-idx' parameter\n");
		return -EINVAL;
	}
	...
	grf = syscon_regmap_lookup_by_phandle(np, "grf");
	if (IS_ERR(grf)) {
	...
	/* Fixed SCL rate of 100kHz */
	rk3x_i2c_set_scl_rate(i2c, 100000);
